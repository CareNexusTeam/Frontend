...
export class InsuranceClaimService {
  ...
  updateStatus(
    ...
    return this.http.patch<InsuranceClaim>(
      `${this.API_URL}/claims/${claimID}/status?status=${status}`,
      {}
    );
  }
  ...
  getByInvoice(invoiceId: number): Observable<InsuranceClaim[]> {
    ...
    return this.http.get<InsuranceClaim[]>(
      `${this.API_URL}/claims/invoice/${invoiceId}`
    );
  }
